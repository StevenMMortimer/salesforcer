...
```
```{r}
created_records_v1 <- sf_create(new_contacts, 
                                object_name = "Contact", 
                                api_type = "Bulk 1.0", 
                                batch_size = 1) # split into 10 batches of 1
created_records_v1

# same ordering of rows!
cbind(created_records_v1, new_contacts)
```
...
```
```{r, eval=FALSE}
created_records_v2 <- sf_create(new_contacts, 
                                object_name = "Contact", 
                                api_type = "Bulk 2.0", 
                                batch_size = 1)
```
...
improvements then switch to the Bulk 2.0 when deleting or querying records.

Here is an example performance benchmark between the Bulk 1.0 and Bulk 2.0 
APIs for a small query. You'll notice that the Bulk 2.0 API is slightly faster. 
In general, the Bulk 2.0 should be faster and the difference in performance 
should grow as size of the queried records grows.

```{r}
soql <- "SELECT Id, Name FROM Contact"
bulk1_query <- function(){sf_query(soql, "Contact", api_type="Bulk 1.0")}
bulk2_query <- function(){sf_query(soql, api_type="Bulk 2.0")} # the Bulk 2.0 does not need the object name

microbenchmark(bulk1_query, bulk2_query, times=10)
```
