...
First, create two new records to be created.
...
```{r}
# add api_type="Bulk 1.0" or api_type="Bulk 2.0" to CRUD and Query calls to use Bulk APIs
object <- "Contact"
...
below we first create 10 records and then show you how the returned results are 
ordered when we process them using the Bulk 1.0 API. After the data is split into 
batches each batch is processed synchronously which preserves the order of the 
rows in the returned output.
