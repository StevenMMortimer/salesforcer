...
so that you are better equipped to understand and use the functions within {salesforcer}.
 ...
 - <a target="_blank" href="https://developer.salesforce.com/docs/atlas.en-us.api_analytics.meta/api_analytics/sforce_analytics_rest_api_get_reportdata.htm">Running a Report Synchronously or Asynchronously</a>  
 - <a target="_blank" href="https://developer.salesforce.com/docs/atlas.en-us.api_analytics.meta/api_analytics/sforce_analytics_rest_api_factmap_example.htm">Understanding a Report Fact Map</a>   
 - <a target="_blank" href="https://developer.salesforce.com/docs/atlas.en-us.api_analytics.meta/api_analytics/sforce_analytics_rest_api_filter_reportdata.htm#example_requestbody_execute_resource">Filter Reports on Demand</a>  
 - <a target="_blank" href="https://developer.salesforce.com/docs/atlas.en-us.api_analytics.meta/api_analytics/sforce_analytics_rest_api_intro.htm">Reports and Dashboards REST API Documentation Home Page</a>  
...
![](./working-with-reports/report-screenshot.png)

The report Id above (`"00O3s000006tDLC"`) is the only information needed to pull 
those same results from an R session, like so: 

```{r simple-report}
my_report_id <- "00O3s000006tDLC"
results <- sf_run_report(my_report_id)
results
```

Note that the `sf_run_report()` function will, by default, run the report
asynchronously. This means that a report instance will be requsted and then the
...
report with different filters applied. This allows you to obtain the results
exactly the results needed without having to create separate copies of the same
...
includes records created prior to last year.
...
the `reportFilters` element in the report's metadata and then reviewing the 
list of report filter operators. 
...
notice that the field names on the report do not match the names of the fields
on the object, so please review carefully the fields on the report. For example, 
the `CREATED_DATE` report field is based on the `CreatedDate` object field.

```{r report-filter-operators}
# report fields
report_fields <- sf_list_report_fields(my_report_id)
names(report_fields$equivalentFieldIndices)
...
repository at: <a target="_blank" href="https://github.com/StevenMMortimer/salesforcer/issues">https://github.com/StevenMMortimer/salesforcer/issues</a>. As a maintainer, reports can be are tough to debug because every Salesforce 
...
query and debug a little bit on your own. Here are a few suggestions:  
  ...
  2. Double check Salesforce's <a target="_blank" href="https://developer.salesforce.com/docs/atlas.en-us.api_analytics.meta/api_analytics/sforce_analytics_rest_api_intro.htm">Reports and Dashboards REST API Developer Guide</a> to see whether if your report type would be supported or limited in some way.
  
  3. Review query unit tests at: <a target="_blank" href="https://github.com/StevenMMortimer/salesforcer/blob/master/tests/testthat/test-report.R">https://github.com/StevenMMortimer/salesforcer/blob/master/tests/testthat/test-reporty.R</a>. 
  ...
  package. The main scripts to review are:  
      ...
      - <a target="_blank" href="https://github.com/StevenMMortimer/salesforcer/blob/master/R/analytics-report.R">https://github.com/StevenMMortimer/salesforcer/blob/master/R/analytics-report.R</a>
      
      - <a target="_blank" href="https://github.com/StevenMMortimer/salesforcer/blob/master/R/utils-report.R">https://github.com/StevenMMortimer/salesforcer/blob/master/R/utils-report.R</a>
