...
file_path1 <- here::here("man", "figures", "cloud.png")
file_path2 <- here::here("man", "figures", "logo.png")
file_path3 <- here::here("man", "figures", "old-logo.png")
...
for (pid in unique(queried_attachments$ParentId)){
  dir.create(here::here(pid), showWarnings = FALSE) # ignore if already exists
}
...
queried_attachments <- queried_attachments %>% 
  #Strategy 1 (ununsed here, but shown as an example)
  mutate(unique_name = paste(Id, Name, sep='___')) %>% 
  #Strategy 2 (ununsed here, but shown as an example)
  mutate(Path = here::here(ParentId))
...
mapply(sf_download_attachment, 
       ...
       queried_attachments$unique_name, 
...
```{r}
# create the attachment metadata required (Name, Path, ParentId)
...
```{r cleanup, include = FALSE}
for (pid in unique(queried_attachments$ParentId)){
  unlink(here::here(pid), recursive=TRUE) # remove directories...
}
...
# object_name argument to add the PNG as a Document instead of an Attachment
document_details <- tibble(Name = "Logo PNG", 
                           Description = "Test Document Create using PNG File", 
                           Body = here::here("man", "figures", "logo.png"),
...
document_details <- tibble(Name = "Data Wrangling Cheatsheet",
                           Description = "RStudio cheatsheet covering dplyr and tidyr.",
                           Url = cheatsheet_url,  
                           FolderId = "00l6A000001EgIwQAK",
                           Keywords = "cheatsheet,document")
result <- sf_create_attachment(document_details, object_name = "Document")
result
```
```{r cleanup, include = FALSE}
